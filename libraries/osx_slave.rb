...
class Chef
  class Resource::JenkinsJNLPSlaveOSX < Resource::JenkinsSlave
    # Chef attributes
    provides :jenkins_jnlp_slave_osx

    # Set the resource name
    self.resource_name = :jenkins_jnlp_slave_osx
...
class Chef
  class Provider::JenkinsJNLPSlaveOSX < Provider::JenkinsSlave
    def load_current_resource
      @current_resource ||= Resource::JenkinsJNLPSlaveOSX.new(new_resource.name)
      super
    end
    ...
    def action_create
      ...
      service_template_resource.run_action(:create)

      service_resource.run_action(:enable)
      # We need to restart the service if the slave jar is updated
      service_resource.run_action(:restart) if slave_jar_resource.updated? or service_template_resource.updated?
    end
    ...
    def service_template_resource
      return @service_template_resource if @service_template_resource

      @service_template_resource = Chef::Resource::Template.new("/Library/LaunchDaemons/#{new_resource.service_name}.plist", run_context)
      @service_template_resource.cookbook 'jenkins_ecosystem'
      @service_template_resource.source 'launchd.erb'
      @service_template_resource.variables(
        service_name: new_resource.service_name,
        java_bin:    new_resource.java_path,
        slave_jar:   slave_jar,
        jnlp_url:    jnlp_url,
    ...
    def service_resource
      ...
      @service_resource = Chef::Resource::MacosxService.new(new_resource.service_name, run_context)
      @service_resource
    end
  end
end
...
Chef::Platform.set(
  resource: :jenkins_jnlp_slave_osx,
  provider: Chef::Provider::JenkinsJNLPSlaveOSX,
)
